"""
Plots composites of Bering Sea Ice Conc compared to selected years
...
ice[np.where(ice<15)]=np.nan

### Meshgrid
lon2,lat2 = np.meshgrid(lon1,lat1)

### Analogs
yearoldq = np.where(years == 2001)[0]

### Baselines
yearnewq = np.where(years == 2018)[0]

iceold = ice[yearoldq,:,:].squeeze()
icenew = ice[yearnewq,:,:].squeeze()
...
ax.annotate(r'\textbf{2001}',
            xy=(0, 0),xytext=(0.5,1.02),xycoords='axes fraction',
            fontsize=38,color='darkgrey',rotation=0,ha='center',va='center')
...
cbar.set_label(r'\textbf{FEBRUARY SEA ICE CONCENTRATION [\%]}',fontsize=13,
                         color='darkgrey',labelpad=-36)
cbar.ax.tick_params(axis='x', size=.001)

ax1.annotate(r'\textbf{2018}',
            xy=(0, 0),xytext=(0.5,1.02),xycoords='axes fraction',
            fontsize=38,color='darkgrey',rotation=0,ha='center',va='center')
...
plt.savefig(directoryfigure + '02-20012018_Bering.png', dpi=300)
